...
@section('content')
    ...
    <h1 class="header-title">Batch ID: <a href="{{ route('batches.show', $batch->ref_number) }}">{{ $batch->ref_number }}</a> - <a href="{{ route('batches.transfer-log', $batch->ref_number) }}">Log</a></h1>
    ...
    {{ Form::model($batch, ['class'=>'form-horizontal', 'url'=>route('batches.transfer', $batch->ref_number)]) }}
...
{{--            {{ Form::checkbox("rows[increment_uid][]", null, ['class'=>'form-control']) }}--}}
{{--            {{ Form::label('increment_uid', 'Weight Based Item', ['class'=>'']) }}--}}
            {{--{{ Form::checkbox("rows[][wt_based]", old("[][wt_based]"), ['class'=>'form-control']) }}--}}
        ...
        <div class="col-2">
            <label class="col-form-label">&nbsp;</label>
            {{ Form::select("rows[uom][]", array_combine(array_keys(config('highline.uom')), array_keys(config('highline.uom'))), old("[uom][]"), ['class'=>'form-control', 'placeholder'=>'-- Select UOM --', 'required'=>'required']) }}
        </div>
        <div class="col-2">
    ...
    {{--<table class="table table-hover">--}}
        ...
        {{----}}
            {{--@foreach($categories as $category)--}}
                {{--@if( ! $category->canTransferTo()) @continue @endif--}}
            {{--<tr>--}}
                ...
                {{--@foreach(config('highline.uom') as $uom)--}}

                    {{--<td>--}}
                        {{--<div class="input-group bootstrap-touchspin">--}}
                            {{--<input id="transfer_qty" type="text" value="" name="transfer_to[{{$category->id}}][{{$uom}}]" class="form-control col-lg-6" placeholder="">--}}
                            {{--<span class="input-group-addon bootstrap-touchspin-postfix">{{ $uom }}</span>--}}
                        {{--</div>--}}
