...
@section('content')
    ...
    <h1 class="header-title">
        Unique Package ID: <a href="{{ route('batches.show', $batch->ref_number) }}">{{ $batch->ref_number }}</a>

    </h1>

    <p>Batch ID: {{ $batch->batch_number }}</p>
    ...
    @foreach($batch->transfer_logs as $transfer_log)
            ...
            <div class="col-4">
                <div class="card-box">
                    ...
                    {{ Form::open(['class'=>'form-horizontal', 'url'=>route('batches.transfer-log', [$batch->ref_number, $transfer_log->id]), 'method'=>'post']) }}
                        ...
                        <ul>
                            @foreach($transfer_log->transfer_log_details as $transfer_log_detail)
                                ...
                                <li style="border-top: solid 1px #ccc;">#{{ $loop->iteration }} - {{ $transfer_log_detail->action }}:
                                    ...
                                    {{ ($transfer_log_detail->batch_created->packaged_date ? ' - Packaged: '.$transfer_log_detail->batch_created->packaged_date->format(config('highline.date_format')) : '' ) }}
                                        <br> <a href="{{ route('batches.show', $transfer_log_detail->batch_created->ref_number) }}">{{ $transfer_log_detail->batch_created->ref_number }}

                                        {!! display_coa_icons($transfer_log_detail->batch_created) !!}
                                    </a><br>
                                    ...
                                    @if($transfer_log_detail->batch_created->wt_based)
                                        ...
                                        @if($transfer_log_detail->batch_created->wt_grams != config('highline.uom.'.$transfer_log_detail->batch_created->uom))
