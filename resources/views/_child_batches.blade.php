...
@foreach($child_batches as $child_batch)
    ...
    <tr>
        ...
        <td style="padding-left: {{ $depth+$indent }}px;">
            <a href="{{ route('batches.show', $child_batch->ref_number) }}">{{ $child_batch->ref_number }}</a>
        </td>

        <td style="padding-left: {{ $depth+$indent }}px; white-space: nowrap;">
            {{ $child_batch->units_purchased }} {{ $child_batch->uom }} @ {{ display_currency($child_batch->unit_price) }}
            @if($child_batch->uom=='g')
                <br><small>({{ number_format($child_batch->units_purchased/config('highline.uom.lb'), 4) }} lb @ {{ display_currency($child_batch->unit_price * config('highline.uom.lb')) }})</small>
            @endif
        </td>
        <td>
            @if($child_batch->order_details->count())
                ...
                @foreach($child_batch->order_details as $order_detail)
                    ...
                    @if(is_null($order_detail->units_accepted) || $order_detail->units_accepted > 0)
                        ...
                        {{ $order_detail->sale_order->txn_date->format('m/d/Y') }}<br>
...
<hr>
    ...
    @if( ! empty($child_batch->created_batch) )
        <tr>
            ...
            <td style="padding-left: {{ $depth+$indent }}px; white-space: nowrap;"><a href="{{ route('batches.show', $child_batch->created_batch->ref_number) }}">{{ $child_batch->created_batch->ref_number }}</a></td>
    ...
    <tr style="background-color: #efefef;">
        ...
        <td>
            ...
            <strong>Inventory: {{ $child_batch->inventory }} {{ $child_batch->uom }}
                ({{ number_format($child_batch->inventory * config('highline.uom')[$child_batch->uom], 4) }} g)</strong><br>
        </td>
        <td>
