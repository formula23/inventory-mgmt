            ...
            foreach($pos as &$po) {
                foreach ($po->batches->groupBy('status') as $status => $items) {
                    foreach ($items->groupBy('uom') as $uom => $items2) {
                        if($uom=='g') {
                            @$units[$po->id]['lb'] += round($items2->sum('units_purchased') / config('highline.uom.lb'), 4)." lb";
                        } else {
                            @$units[$po->id][$uom] += $items2->sum('units_purchased')." ".$uom;
                        }
                        @$units_purchased[$customer_type][$status][$uom] += $items2->sum('units_purchased');
                        @$units_purchased_total[$customer_type][$status]['total_grams'] += ($items2->sum('units_purchased') * config('highline.uom')[$uom]);
                    }
                }
            }
        }
