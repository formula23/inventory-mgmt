...
//            $item['brand_id']=( ! empty($brands[$item[1]])?$brands[$item[1]]:null);

            $item["tax_rate_qty"] = $tax_rate_quantity;
//            $item['unit_tax_amount'] = $unit_tax_amount;

//            $item['tax_amount'] = round($tax_rate_quantity * $unit_tax_amount, 2);
            $item['unit_cost'] = round($item['Pre-Tax Unit Cost'], 2);
            $item['unit_tax_amount'] = $unit_tax_amount;
            $item['subtotal'] = round($item['Qty'] * ($item['Pre-Tax Unit Cost'] + $unit_tax_amount), 2);
