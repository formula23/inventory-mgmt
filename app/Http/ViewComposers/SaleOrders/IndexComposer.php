...
//        $sale_orders->load(['order_details','order_details.batch']);
        ...
        foreach($sale_orders as $sale_order) {
            ...
            foreach($sale_order->order_details_cog as $order_detail) {
                if(empty($order_detail->batch)) continue;
        ...
        $total_lbs_sold = array_sum($total_grams_sold) / config('highline.conversions.grams_per_pound');
        ...
        foreach($units_purchased as $sid => &$units) {
            ...
            foreach($units as $uom=>$v) {
                ...
                try {
                    ...
                    if($v) {
                        if(empty($totals[$sid])) { $totals[$sid]['grams']=0; }
            ...
            if(!empty($totals[$sid])) {
                $totals[$sid]['lbs'] = round($totals[$sid]['grams'] / config('highline.conversions.grams_per_pound'), 2);
            }
