...
//        dump(Auth::user()->id);
        if( ! $this->request->filters) {
            $this->request->filters = ( Cache::has($this->cache_key) ? Cache::get($this->cache_key) : $this->default_filters );
        } else {
            $this->setCache();
        }
        ...
        if($filters->isEmpty()) {
            $this->request->filters = $this->default_filters;
    ...
    {
        if($this->cache_key) Cache::forever($this->cache_key, $this->request->filters);
    }
}